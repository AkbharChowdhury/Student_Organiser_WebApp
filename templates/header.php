...
const LOGO = 'fas fa-book-reader';
const LOGO_TEXT = 'StudentPlanner';
const PAGES = ['index', 'login', 'register'];
const USER = ['profile', 'coursework', 'teachers', 'modulesAndTeacher', 'campus', 'calendar', 'dashboard'];
const ADMIN = ['admin_dashboard'];

define("USER_AND_ADMIN_PAGES", array_merge(USER, ADMIN));
...
require_once in_array($current_page, USER_AND_ADMIN_PAGES) ? '../includes/class-autoload.php' : 'includes/class-autoload.php';
...
define('FILE_PATH', [
    ...
    'logo' => in_array($current_page, USER_AND_ADMIN_PAGES) ? '../img/logo.png' : 'img/logo.png',
    'calendar' => in_array($current_page, USER) ? '../calendar/calendar.php' : 'calendar/calendar.php',
    'css' => in_array($current_page, USER_AND_ADMIN_PAGES) ? '../css/style.css' : 'css/style.css',
    'script' => in_array($current_page, USER_AND_ADMIN_PAGES) ? '../js/script.js' : 'js/script.js',
    ...
    'logout' => in_array($current_page, USER_AND_ADMIN_PAGES) ? '../logout.inc.php' : 'logout.inc.php',
    ...
    'dashboard' => in_array($current_page, USER_AND_ADMIN_PAGES) ? '../dashboard/dashboard.php' : 'dashboard/dashboard.php',
    'admin_dashboard' => in_array($current_page, USER_AND_ADMIN_PAGES) ? '../admin/admin_dashboard.php' : 'admin/admin_dashboard.php',

]);


if (!Helper::studentIsLoggedIn() && in_array($current_page, USER_AND_ADMIN_PAGES)) Helper::userIsLoggedIn(FILE_PATH['login']);
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                ...
                <?php if (!Helper::studentIsLoggedIn() && !Helper::adminIsLoggedIn()) :  // public navbar links
                    ?>
                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('index') ?>" <?= Helper::ariaCurrent('index') ?>
                           href="<?= !Helper::studentIsLoggedIn() ? 'index.php' : 'admin/' ?>">Home</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('contact') ?>" <?= Helper::ariaCurrent('contact') ?>
                           href="index.php?contact#contact" data-target="#contact">Contact</a>
                    </li>

                <?php elseif (Helper::adminIsLoggedIn()) : ?>
                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('admin_dashboard') ?>" <?= Helper::ariaCurrent('admin_dashboard') ?>
                           href="<?= FILE_PATH['admin_dashboard'] ?>">Dashboard</a>
                    </li>
                ...
                <?php else : // student logged-in pages
                    ?>
                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('dashboard') ?>" <?= Helper::ariaCurrent('dashboard') ?>
                           href="<?= FILE_PATH['dashboard'] ?>">Dashboard</a>
                    </li>
                ...
                <?php if (Helper::studentIsLoggedIn()) : // nav-links for when user is logged in
                    ...
                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('coursework') ?>" <?= Helper::ariaCurrent('coursework') ?>
                           href="<?= FILE_PATH['coursework'] ?>">Coursework</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('modulesAndTeacher') ?>" <?= Helper::ariaCurrent('modulesAndTeacher') ?>
                           href="<?= FILE_PATH['modulesAndTeachers'] ?>">Modules & Teachers</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('calendar') ?>" <?= Helper::ariaCurrent('calendar') ?>
                           href="<?= FILE_PATH['calendar'] ?>">Calendar</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link <?= Helper::activeLink('campus') ?>" <?= Helper::ariaCurrent('campus') ?>
                           href="<?= FILE_PATH['campus'] ?>">Campus</a>
                    </li>
            ...
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item">
                    <a class="nav-link <?= Helper::activeLink('login') ?>" <?= Helper::ariaCurrent('login') ?>
                       href="login.php">Login</a>
                </li>
                <button class="btn btn-success" type="button" onclick="window.location.href='register.php'">Register
                </button>
            </ul>
            <?php else : ?>
                ...
                <li class="nav-item dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-menu-darj dropdown-menu-end" aria-labelledby="navbarDropdown">
                        <?php if (Helper::studentIsLoggedIn()) :  // show profile link page for student?>
                            <li>
                                <a class="dropdown-item <?= Helper::activeLink('profile') ?>"
                                   href="<?= FILE_PATH['profile'] ?>">My profile</a>
                            </li>
                            <li>
